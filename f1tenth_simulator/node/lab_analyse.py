...
class analyze:
    ...
    def __init__(self,racecar_name='racecar',vesc="vesc"):
        ...
        self.ackermann_stamped=Subscriber(driver_topic,AckermannDriveStamped)
        ...
        ani = animation.FuncAnimation(self.fig, self.animate, fargs=(self.vehicle_heading, self.times,self.x,self.y,self.velocity), interval=1000)
        plt.show()
    ...
    def master_callback(self,odom_msg,ackermann_msg): 
        ...
        if(self.count>2):
            if(abs(self.previous_vehicle_heading - rpy[2])>0.3):
                rospy.logwarn("discontinuity, {}, {}".format(rpy[2],self.previous_vehicle_heading))
                if(self.killatDiscontinuity):
                    self.reset()
                


        print("x:",x,"y:",y,"speed:",speed,"heading to:",rpy[2],"u:",u,"delta:",delta,"time:",time)

        if(self.count>0):
            self.vehicle_heading.append(rpy[2])
            self.times.append(time)
            self.x.append(x)
            self.y.append(y)
            self.velocity.append(speed)
            self.previous_vehicle_heading = rpy[2]
    ...
    def reset(self):
        self.previous_vehicle_heading = None 
        self.x = []
        self.y = []
        self.velocity = []
        self.times=[]
    ...
    def animate(self,i,times,vehicle_heading,x,y,velocity):
        # Limit x and y lists to window items
        self.vehicle_heading = self.vehicle_heading[-self.window:]
        ...
        self.ax.plot(self.times, self.vehicle_heading)
        self.ax2.plot(self.times, self.velocity)
        self.ax3.plot(self.x, self.y)
        

        self.ax.set_title("vehicle_heading vs time")
